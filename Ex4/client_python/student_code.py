...
min_y, min_x = sys.float_info.max
max_y, max_x = sys.float_info.min
...
while client.is_running() == 'true':
    pokemons = load_pokemons(client.get_pokemons())
    # pokemons = [p.Pokemon for p in pokemons]
    for p in pokemons:
        # x, y, _ = p.pos.split(',')
        x = p.pos[0]
        y = p.pos[1]
        scale_x = my_scale(float(x), x=True)
        scale_y = my_scale(float(y), y=True)
        tup = (scale_x, scale_y, 0)
        p.set_pos(tup)
    ...
    #check which agent what pokemons are closet and send him there
    pokemons = load_pokemons(client.get_pokemons())
    agents = load_agents(client.get_agents())
